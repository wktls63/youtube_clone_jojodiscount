function getVideoList() {
    ...
    imgs.onreadystatechange = function() {
        // 요청이 완료되었을 때(readyState === XMLHttpRequest.DONE)
        if (imgs.readyState === XMLHttpRequest.DONE) {
            // 요청이 성공적으로 처리되었을 때(status === 200)
            if (imgs.status === 200) {
                // 서버에서 받은 JSON 형식의 응답을 JavaScript 객체로 변환
                let data = JSON.parse(imgs.responseText);
    ...
    // 서버로 GET 요청을 보냄
    xhr.open('GET', 'http://oreumi.appspot.com/video/getVideoList', +"https://storage.googleapis.com/oreumi.appspot.com/img_0.jpg",true);
    xhr.send();
}